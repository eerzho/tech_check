...
func (p *Parser) GetHeaderIP(r *http.Request) string {
	forwardedFor := r.Header.Get(constants.HeaderForwardedFor.String())
	if forwardedFor != "" {
...
func (p *Parser) GetAuthUser(r *http.Request) (*models.User, error) {
	user, ok := r.Context().Value(constants.ContextAuthUser).(*models.User)
	if !ok {
		return nil, constants.ErrInvalidUserType
	}
