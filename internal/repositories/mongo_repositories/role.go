...
func NewRole(db *mongo.Database) *Role {
	return &Role{
		maxListCount: 200,
		collection:   db.Collection(constants.TableRoles.String()),
	}
}
...
func (r *Role) Create(ctx context.Context, role *models.Role) error {
...
func (r *Role) Delete(ctx context.Context, id string) error {
	...
	userCollection := r.collection.Database().Collection(constants.TableUsers.String())
