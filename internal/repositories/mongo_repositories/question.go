...
func NewQuestion(db *mongo.Database) *Question {
	return &Question{
		maxListCount: 200,
		collection:   db.Collection(constants.TableQuestions.String()),
	}
}
...
func (q *Question) List(ctx context.Context, page, count int, filters, sorts map[string]string) ([]models.Question, *dto.Pagination, error) {
	...
	for key, value := range filters {
		if key == "text" {
			...
			_, err := constants.ValidateGradeName(value)
			if err == nil {
				filter[key] = value
			}
		}
...
func (q *Question) Create(ctx context.Context, question *models.Question) error {
...
func (q *Question) Update(ctx context.Context, question *models.Question) error {
...
func (q *Question) GetRandom(ctx context.Context, category *models.Category, grade constants.GradeName, count int) ([]models.Question, error) {
